...
inline void tri2hybridBFS( PolySurfMesh& pmesh ) {
    ...
    for( const auto& [w, eh, rank]: edges ) {
        ...
        while( !queue.empty() ) {
            ...
            std::sort( nextEhs.begin(), nextEhs.end(), [&pmesh, &eRank]( const OpenMesh::EdgeHandle& a, const OpenMesh::EdgeHandle& b ) {
                return pmesh.property( eRank, a ) < pmesh.property( eRank, b );
            } );
...
void RatRace::postProcessing( const int& nQuads, SizeGrid* sizegrid, const std::experimental::filesystem::path& outputPath ) {
    ...
    LOG( INFO ) << "Splitted boundary val2: " << valence2SplitBoundary();
    LOG( INFO ) << "Removed boundary val2: " << removeBoundaryValence2Vertices();
    LOG( INFO ) << "Removed boundary val2: " << removeBoundaryValence2Vertices();
    while( true ) {
        int removed = removeInteriorValence2Vertices();
        if( removed == 0 )
...
int RatRace::swapValence() {
    ...
    do {
        ...
        for( auto eh: mesh_.edges() ) {
            ...
            if( !mesh_.property( vRemesh, vhs[0] ) && !mesh_.property( vRemesh, vhs[1] ) && !mesh_.property( vRemesh, vhs[2] ) &&
                !mesh_.property( vRemesh, vhs[3] ) && !mesh_.property( vRemesh, vhs[4] ) && !mesh_.property( vRemesh, vhs[5] ) ) {
                continue;
            }
